...
class CBAM(nn.Module):
    def __init__(self, channel, redection=16):
        super(CBAM, self).__init__()
        self.ChannelAttentionModule = ChannelAttentionModule(channel=channel, reduction=redection)
        self.SpatialAttentionModule = SpatialAttentionModule(kernel_size=7, stride=1, padding=3)

    def forward(self, x):
        x1 = self.ChannelAttentionModule(x)
        x = x * x1
