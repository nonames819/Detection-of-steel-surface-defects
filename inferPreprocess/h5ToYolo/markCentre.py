...
def find_centre(img):  # 输入8位灰度图像，高宽格式(H,W)=(800,1280)。返回标注了检测出的圆及圆心的灰度图片
    ...
    # 识别铝片外圆
    img_outer = cv.medianBlur(im0, 3)
    # img_outer = im0  # 不模糊处理会导致边缘检测速度慢
    circles_outer = cv.HoughCircles(img_outer, cv.HOUGH_GRADIENT, dp=2, param1=30, param2=50,
                                    minRadius=300, maxRadius=400, minDist=1)

    '''******************************！！！以下这段代码是程序速度的瓶颈，耗时与candidates数成正比！！！******************************'''
    # 筛选最优预测圆
    find_flag = False
    centre = None
    if circles_outer is not None and circles_inner is not None:
        circles_inner = np.uint16(np.around(circles_inner))  # 把小圆的圆心和半径的值转为整型
        circles_outer = np.uint16(np.around(circles_outer))  # 把大圆的圆心和半径的值转为整型
        # print(circles_outer.shape[1])

        # 筛选最优小圆
        best_inner_circle = draw_optimal_circle(img_blur=img_inner, circles=circles_inner, candidates=5, overlap_rate=5)
        # best_outer_circle = None

        # 筛选最优大圆
        best_outer_circle = draw_optimal_circle(img_blur=img_outer, circles=circles_outer, candidates=5, overlap_rate=5)
        # best_inner_circle = None

        '''******************************！！！以上这段代码是程序速度的瓶颈，耗时与candidates数成正比！！！******************************'''

        if best_outer_circle is not None and best_inner_circle is not None:  # 可视化预测圆
            x1, y1, r1 = best_inner_circle
            x2, y2, r2 = best_outer_circle
            if np.square(int(x2)-int(x1)) + np.square(int(y2)-int(y1)-300) <= 7:  # 当大小圆的圆心足够接近时才视为有效圆心
                find_flag = True
                centre = (int((x1+x2)/2), int((y1+300+y2)/2))
                cv.circle(im0, (x1, y1+300), r1, 255, 1)   # 画小圆
                cv.circle(im0, (x2, y2), r2, 255, 1)   # 画大圆
                cv.circle(im0, (int((x1+x2)/2), int((y1+300+y2)/2)), 2, 255, 1)   # 画圆心
...
def mark_centre(indir, outdir):
    process_num = 3  # 进程数，indir下的事件帧文件总数必须为process_num的整数倍，process_num数应小于cpu可用核数
...
if __name__ == '__main__':
    mark_centre(indir="../../TestASet/data_stage1", outdir="../../TestASet/data_stage2")
